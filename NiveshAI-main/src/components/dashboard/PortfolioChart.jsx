    ...
    <Card>
      ...
      <CardContent>
        <div className="h-64 flex items-center justify-center text-muted-foreground text-sm">
          {message || 'Make your first trade to see portfolio performance'}
        </div>
      </CardContent>
    </Card>
  );
...
export default function PortfolioChart({ trades, stocks, equityData }) {
  // Use pre-computed equity curve from portfolioEngine if available
  if (equityData && equityData.length > 0) {
    const chartData = equityData.map(d => ({ date: d.date?.slice(5) || '', value: d.value }));
    return <ChartCard chartData={chartData} />;
  }

  // Fallback: simulate from trades
  const holdings = {};
  (trades || []).forEach(t => {
  ...
  if (!firstStock?.ohlcv_data?.length) return <EmptyChart />;

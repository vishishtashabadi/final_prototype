...
// Target: First-time retail investors — no jargon!
// ─────────────────────────────────────────────────────────────
export function generateSignalReasoning(analysis) {
  const { signal, details, marketState, regime, divergence, confirmations, riskReward } = analysis;

  const reasons = [];

  // Overall signal summary (plain language)
  if (signal === 'STRONG_BUY') reasons.push('The stock is showing strong signs that it may go up soon. Multiple indicators agree on this.');
  else if (signal === 'BUY') reasons.push('The stock looks like it could be a good time to buy. Most indicators are positive.');
  else if (signal === 'SELL') reasons.push('The stock may be due for a drop. It might be wise to sell or wait before buying.');
  else if (signal === 'STRONG_SELL') reasons.push('Multiple warning signs are flashing. The stock is likely to go down.');
  else reasons.push('The signals are mixed right now. It is safer to wait and watch.');

  // RSI — "Is the stock too high or too low?"
  if (details.rsi) {
    const val = details.rsi.value;
    if (val < 35) reasons.push(`The stock has dropped a lot recently (a measure called RSI is at ${val?.toFixed(0)}), meaning it might be undervalued and could bounce back.`);
    else if (val > 65) reasons.push(`The stock has risen a lot recently (a measure called RSI is at ${val?.toFixed(0)}), meaning it might be overvalued and could dip.`);
  }

  // MACD — "Is buying or selling momentum increasing?"
  if (details.macd?.status === 'BULLISH') reasons.push('Recent buying momentum is picking up speed — a positive sign.');
  else if (details.macd?.status === 'BEARISH') reasons.push('Recent selling pressure is increasing — a cautious sign.');

  // EMA Crossover — "Short-term vs long-term trend"
  if (details.ema?.status === 'BULLISH') reasons.push('Short-term momentum is stronger than the long-term trend, which is encouraging.');
  else if (details.ema?.status === 'BEARISH') reasons.push('Short-term momentum is slowing down compared to the long-term trend, suggesting caution.');

  // Bollinger Bands — "Is the price at an extreme?"
  if (details.bollinger?.squeeze) reasons.push('The stock is in a tight range — a bigger move may happen soon.');
  else if (details.bollinger?.status === 'BULLISH') reasons.push('The price is near its lower range, which sometimes means it is undervalued and may rise.');
  else if (details.bollinger?.status === 'BEARISH') reasons.push('The price is near its upper range, which sometimes means it is overvalued and may fall.');

  // ADX — "Trend strength"
  if (details.adx?.status === 'BULLISH') reasons.push(`The uptrend is strong (trend strength: ${details.adx.adx?.toFixed(0)}), meaning the upward move has solid backing.`);
  else if (details.adx?.status === 'BEARISH') reasons.push(`The downtrend is strong (trend strength: ${details.adx.adx?.toFixed(0)}), meaning the downward move has solid backing.`);

  // Volume — "Are people actually trading?"
  if (details.volume?.status === 'BULLISH') reasons.push(`Trading volume is high (${details.volume.ratio?.toFixed(1)}x normal), which confirms that the current move is real and not just noise.`);

  // Market regime — "What's the big picture?"
  ...
  // Signal strength
  reasons.push(`Signal strength: ${confirmations} out of 8 indicators confirmed.`);
